classdef LineMapLocalizerRelaxed < handle
    ...
    methods(Access = public)
        function obj = LineMapLocalizerRelaxed(lines_p1,lines_p2,gain,errThresh,gradThresh)
            % create a LineMapLocalizerRelaxed
       ...
       function ids = throwInliers(obj,pose,scan_homogeneous)
        ...
        function avgErr = fitError(obj,pose,ptsInModelFrame)
            ...
            err = sum(r2);
            num = length(r2);
            if(num >= LineMapLocalizerRelaxed.minPts)
                avgErr = sqrt(err)/num;
            else
